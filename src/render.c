...
/*   Updated: 2026/01/15 12:30:53 by almaldon         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void draw_minimap(t_mlx *mlx, t_map_data *map, t_player_data *player)
{
    ...
    for (i = 0; i < map_h; i++)
    {
        ...
        for (j = 0; j < map_w; j++)
        {
            ...
            if (map->only_map[i][j] == '1')
                color = 0xFFFFFF; // blanco para pared
            else
                color = 0x000000; // negro para suelo
    ...
    for (int y = -2; y <= 2; y++)
        for (int x = -2; x <= 2; x++)
            my_mlx_pixel_put(mlx, px + x, py + y, 0xFF0000); // rojo
}
...
void render_scene(t_game *game)
{
	...
	t_ray ray;
	t_texture *tex;
	float FOV = game->map_data.player_data.fov * (M_PI / 180.0f); // en radianes
	...
	// 2️⃣ Raycasting columna por columna
	for (col = 0; col < game->mlx.win_w; col++)
	{
		...
		// Debug: imprimir valores del rayo
		// ft_printf("col=%d, hit_x=%f, hit_y=%f, distance=%f\n",
		//           col, ray.hit_x, ray.hit_y, ray.distance);
		...
		// Elegir textura según el lado que se impactó
		if (ray.hit_vertical)
			tex = (ray.ray_dir_x > 0) ? &game->we : &game->ea;
		else
			tex = (ray.ray_dir_y > 0) ? &game->no : &game->so;

		// Calcular posición horizontal en la textura
		float hit_pos;
		if (ray.hit_vertical)
		...
		// 3️⃣ Dibujar columna de la pared con la textura
		for (y = y_start; y <= y_end; y++)
		{
			// Mapear la coordenada vertical al pixel de la textura
